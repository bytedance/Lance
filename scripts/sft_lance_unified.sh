...
# ==============================手动启动模式==============================
MANUAL_START=True  # True: 手动启动; False: 自动启动

if [ "$MANUAL_START" = "True" ]; then
    # for 1台机器
    export ARNOLD_WORKER_NUM=1 # 机器数量
    export ARNOLD_WORKER_GPU=8 # HACK一下
    export ARNOLD_ID=0 # 机器ID
    MAIN_PROCESS_IP=127.0.0.1
    MAIN_PROCESS_PORT=${ARNOLD_WORKER_0_PORT:-6668}
    MACHINE_RANK=$ARNOLD_ID

    # # for 多台机器
    # export ARNOLD_WORKER_NUM=1 # 2 # 机器数量
    # OFFSET=0
...
# ==============================分布式配置==============================
export NCCL_DEBUG=WARN  # 只打印错误日志
...
VAE_MODEL_TYPE="wan"
VISUAL_UND=True
VISUAL_GEN=True # False # True，如果只有理解任务，关闭生成任务

# ==============================冻结配置==============================
FREEZE_LLM=False
FREEZE_LLM_EMBED_TOKENS=True
FREEZE_UND_PARAMS=False
FREEZE_UND=False

# ==============================数据配置==============================
DATASET_CONFIG_FILE=${DATASET_CONFIG_FILE:-config/train_local/unified.yaml}
VAL_DATASET_CONFIG_FILE=${VAL_DATASET_CONFIG_FILE:-config/train_local/unified.yaml}

NUM_WORKERS=8
PREFETCH_FACTOR=2
MAX_NUM_TOKENS=65536
EXPECTED_NUM_TOKENS=65536
MAX_NUM_TOKENS_PER_SAMPLE=76800
PREFER_BUFFER_BEFORE=76800
MAX_BUFFER_SIZE=50
...
WANDB_NAME=${DATE}_"$NUM_SHARD"x"$NUM_REPLICATE"_unified
WANDB_OFFLINE=True # False

OUTPUTS_DIR=./outputs/${WANDB_PROJECT}

# ==============================验证配置==============================
VALIDATION_STEP=-1 # 100 ｜ -1表示不验证
